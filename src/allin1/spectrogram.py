...
# ### ALTERMATIVE VERSION TEST
import numpy as np
from pathlib import Path
from typing import List, Tuple
from tqdm import tqdm
from multiprocessing import Pool
from madmom.audio.signal import FramedSignalProcessor, Signal, AudioReaderProcessor
from madmom.audio.stft import ShortTimeFourierTransformProcessor
from madmom.processors import SequentialProcessor
from madmom.audio.spectrogram import FilteredSpectrogramProcessor, LogarithmicSpectrogramProcessor
...
def extract_spectrograms(demix_paths: List[Path], spec_dir: Path, multiprocess: bool = True):
  ...
  if todos:
    ...
    spec = LogarithmicSpectrogramProcessor(mul=1, add=1)
    audio_reader = AudioReaderProcessor()
    processor = SequentialProcessor([audio_reader, frames, stft, filt, spec])
...
def _extract_spectrogram(args: Tuple[Path, Path, SequentialProcessor]):
  ...
  audio_source = AudioDataSource(str(src), num_channels=4, sample_rate=44100)
  spec = processor(audio_source)
